...
import require$$8, { join, normalize, resolve, sep, dirname, extname } from "node:path";
import require$$2$1, { readFileSync, existsSync } from "node:fs";
...
function requireUtf32() {
  ...
  Utf32Encoder.prototype.write = function(str) {
    ...
    var dst = Buffer2.alloc(src2.length * 2 + 4);
...
function requireDist$3() {
  ...
  function parse2(header, options) {
    const stopChar = (options == null ? void 0 : options.comma) === true ? COMMA : 65536;
    const len = header.length;
    let index = skipOWS(header, (options == null ? void 0 : options.start) ?? 0, len);
    const valueStart = index;
    index = skipValue(header, index, len, stopChar);
    const valueEnd = trailingOWS(header, valueStart, index);
    const type2 = header.slice(valueStart, valueEnd).toLowerCase();
    if ((options == null ? void 0 : options.parameters) === false) {
...
function requireMimeTypes() {
  ...
  (function(exports) {
    ...
    var extname2 = require$$0$5.extname;
...
function requireTypeIs() {
  ...
  hasRequiredTypeIs = 1;
  var contentType2 = requireDist$3();
  ...
  function normalizeType(value) {
    ...
    var type2 = contentType2.parse(value, { parameters: false }).type;
...
function requireUtils$2() {
  ...
  const contentType2 = requireDist$2();
  ...
  function getCharset(req) {
    ...
    return (_a = contentType2.parse(header).parameters.charset) == null ? void 0 : _a.toLowerCase();
  }
  ...
  function normalizeOptions(options, defaultType) {
    ...
    const inflate = (options == null ? void 0 : options.inflate) !== false;
    const limit = typeof (options == null ? void 0 : options.limit) === "undefined" || (options == null ? void 0 : options.limit) === null ? 102400 : bytes2.parse(options.limit);
    const type2 = (options == null ? void 0 : options.type) || defaultType;
    const verify = (options == null ? void 0 : options.verify) || false;
    const defaultCharset = (options == null ? void 0 : options.defaultCharset) || "utf-8";
    if (limit === null) {
    ...
    const shouldParse = typeof type2 !== "function" ? typeChecker(type2) : type2;
    return {
      inflate,
      limit,
      verify,
...
function requireRead() {
  ...
  const getBody = requireRawBody();
  const iconv = requireLib$1();
  const onFinished2 = requireOnFinished();
  const zlib = require$$4$1;
  const hasBody = requireTypeIs().hasBody;
  const { getCharset } = requireUtils$2();
  read_1 = read;
  function read(req, res, next, parse2, debug, options) {
    ...
    if ((options == null ? void 0 : options.skipCharset) !== true) {
      ...
      if (!!(options == null ? void 0 : options.isValidCharset) && !options.isValidCharset(encoding2)) {
  ...
  function contentstream(req, debug, inflate) {
    const encoding2 = (req.headers["content-encoding"] || "identity").toLowerCase();
    ...
    const stream = createDecompressionStream(encoding2, debug);
    req.pipe(stream);
    return stream;
  }
...
function requireJson() {
  ...
  hasRequiredJson = 1;
  const debug = requireSrc()("body-parser:json");
  const read = requireRead();
  const { normalizeOptions } = requireUtils$2();
  json_1 = json;
  ...
  function createJsonParser(options) {
    const reviver = options == null ? void 0 : options.reviver;
...
function requireRaw() {
  ...
  hasRequiredRaw = 1;
  const debug = requireSrc()("body-parser:raw");
  const read = requireRead();
  const { normalizeOptions, passthrough } = requireUtils$2();
  raw_1 = raw;
...
function requireText() {
  ...
  hasRequiredText = 1;
  const debug = requireSrc()("body-parser:text");
  const read = requireRead();
  const { normalizeOptions, passthrough } = requireUtils$2();
  text_1 = text;
...
function requireUtil_inspect() {
  ...
  util_inspect = require$$0$4.inspect;
  return util_inspect;
}
var objectInspect;
...
function requireUrlencoded() {
  ...
  const createError = requireHttpErrors();
  const debug = requireSrc()("body-parser:urlencoded");
  const read = requireRead();
  ...
  function createQueryParser(options) {
    const extended = Boolean(options == null ? void 0 : options.extended);
    let parameterLimit = (options == null ? void 0 : options.parameterLimit) !== void 0 ? options == null ? void 0 : options.parameterLimit : 1e3;
    const charsetSentinel = options == null ? void 0 : options.charsetSentinel;
    const interpretNumericEntities = options == null ? void 0 : options.interpretNumericEntities;
    const depth = extended ? (options == null ? void 0 : options.depth) !== void 0 ? options == null ? void 0 : options.depth : 32 : 0;
    ...
    return function parse2(body, encoding2) {
...
function requireBodyParser() {
  ...
  (function(module, exports) {
    ...
    exports.raw = requireRaw();
    exports.text = requireText();
    exports.urlencoded = requireUrlencoded();
...
function requireDist$1() {
  ...
  dist$1.PathError = dist$1.TokenData = void 0;
  dist$1.parse = parse2;
  dist$1.compile = compile;
  dist$1.match = match;
  dist$1.pathToRegexp = pathToRegexp;
  dist$1.stringify = stringify;
...
function requireRouter() {
    ...
    function next(err) {
      ...
      processParams(self2.params, layer2, paramcalled, req, res, function(err2) {
        if (err2) {
          ...
          trimPrefix(layer2, layerError, layerPath, path);
        }
        sync = 0;
      });
    }
    function trimPrefix(layer2, layerError, layerPath, path) {
  ...
  Router.prototype.route = function route2(path) {
    const route3 = new Route(path);
    const layer2 = new Layer(path, {
      sensitive: this.caseSensitive,
      strict: this.strict,
      end: true
...
function requireApplication() {
  ...
  (function(module, exports) {
    ...
    var resolve2 = require$$8.resolve;
...
function requireAccept() {
  ...
  function parseAccept(header) {
    ...
    while (index < header.length) {
      ...
      var parsed = contentType2.parse(header, { comma: true, start });
...
function requireCharset() {
  ...
  function parseAcceptCharset(accept2) {
    var accepts2 = parseAccept(accept2);
    for (var i = 0, j = 0; i < accepts2.length; i++) {
      var charset2 = formatCharset(accepts2[i], i);
  ...
  function preferredCharsets(accept2, provided) {
    var accepts2 = parseAcceptCharset(accept2 === void 0 ? "*" : accept2 || "");
    if (!provided) {
...
function requireEncoding() {
  ...
  function parseAcceptEncoding(accept2) {
    var accepts2 = parseAccept(accept2);
    var hasIdentity = false;
    var minQuality = 1;
    for (var i = 0, j = 0; i < accepts2.length; i++) {
      var encoding2 = formatEncoding(accepts2[i], i);
      if (encoding2) {
  ...
  function preferredEncodings(accept2, provided, preferred) {
    var accepts2 = parseAcceptEncoding(accept2 || "");
...
function requireLanguage() {
  ...
  function parseAcceptLanguage(accept2) {
    var accepts2 = parseAccept(accept2);
    for (var i = 0, j = 0; i < accepts2.length; i++) {
      var language2 = formatLanguage(accepts2[i], i);
  ...
  function specify(language2, spec, index) {
    var p = formatLanguage(contentType2.parse(language2), 0);
    if (!p) return null;
    var s = 0;
  ...
  function preferredLanguages(accept2, provided) {
    var accepts2 = parseAcceptLanguage(accept2 === void 0 ? "*" : accept2 || "");
    if (!provided) {
...
function requireMediaType() {
  ...
  function parseAccept(accept2) {
    var accepts2 = parseAcceptHeader(accept2);
    for (var i = 0, j = 0; i < accepts2.length; i++) {
      var mediaType2 = formatMediaType(accepts2[i], i);
  ...
  function specify(type2, spec, index) {
    var p = formatMediaType(contentType2.parse(type2), 0);
    var s = 0;
    if (!p) {
      return null;
    }
  ...
  function preferredMediaTypes(accept2, provided) {
    var accepts2 = parseAccept(accept2 === void 0 ? "*/*" : accept2 || "");
    if (!provided) {
...
function requireRequest() {
  ...
  req.accepts = function() {
    var accept2 = accepts2(this);
    return accept2.types.apply(accept2, arguments);
  };
  req.acceptsEncodings = function() {
    var accept2 = accepts2(this);
    return accept2.encodings.apply(accept2, arguments);
  };
  req.acceptsCharsets = function() {
    var accept2 = accepts2(this);
    return accept2.charsets.apply(accept2, arguments);
  };
...
function requireContentDisposition() {
  ...
  function decodefield(str) {
    ...
    const charset2 = match[1].toLowerCase();
...
function requireSend() {
  ...
  SendStream.prototype.pipe = function pipe(res) {
    ...
    if (root !== null) {
      ...
      parts = normalize2(path2).split(sep2);
...
function requireServeStatic() {
  ...
  var escapeHtml = requireEscapeHtml();
  var parseUrl = requireParseurl();
  var resolve2 = require$$0$5.resolve;
  var send = requireSend();
  var url = require$$0$7;
...
function loadConfig() {
  const candidates = [join(process.cwd(), "contact-form.config.json"), join(process.cwd(), "src/lib/contact-form.config.json")];
...
function getVisitorIp(req) {
  ...
  if (typeof xff === "string" && xff.trim()) {
    return ((_a = xff.split(",")[0]) == null ? void 0 : _a.trim()) ?? "";
  }
  return ((_b = req.socket) == null ? void 0 : _b.remoteAddress) ?? req.ip ?? "unknown";
}
async function handler$1(req, res) {
  ...
  if (body == null ? void 0 : body._gotcha) {
  ...
  const email = (_a = body == null ? void 0 : body.user) == null ? void 0 : _a.email;
  const mobile = (_b = body == null ? void 0 : body.user) == null ? void 0 : _b.mobile;
  if (!email && !mobile) {
    res.status(400).json({
      success: false,
      error: "user.email or user.mobile is required"
    });
    return;
  }
  const messageBody = (_e = (_d = (_c = body == null ? void 0 : body.conversation) == null ? void 0 : _c.messages_attributes) == null ? void 0 : _d[0]) == null ? void 0 : _e.body;
  if (!messageBody) {
    res.status(400).json({
      success: false,
      error: "conversation message body is required"
    });
    return;
  }
  const config = getConfig();
  if (!(config == null ? void 0 : config.forms)) {
    res.status(500).json({
      success: false,
  ...
  const formConfig = Object.hasOwn(config.forms, formName) ? config.forms[formName] : void 0;
  if (!formConfig) {
    res.status(404).json({
      success: false,
      error: `Form "${formName}" not configured`
    });
    return;
  }
  ...
  const payload = {
    conversation: {
      ...
      data: {
        ...
        ...(_f = body.conversation) == null ? void 0 : _f.data,
        __gd_type: "contact",
        ...formConfig.overrideEmail && {
          __gd_override_email: formConfig.overrideEmail
        }
      }
    }
  };
  ...
  try {
    ...
    if (!response2.ok) {
      ...
      console.error(`[contact] inbox backend ${response2.status}:`, text.slice(0, 500));
      res.status(502).json({
        success: false,
...
const EMPTY_ADSENSE_RUNTIME_CONFIG = {
  publisherId: null,
  scriptHtml: "",
  adsTxt: null,
  appAdsTxt: null
};
function buildCanonicalAdSenseScript(publisherId) {
...
function loadAdSenseRuntimeConfig(baseDir) {
...
function resolveRuntimeManifestPath(baseDir) {
...
function resolveAdSenseTextFile(config, key) {
  return config[key];
}
...
function isRuntimeManifestShape(value) {
  if (typeof value !== "object" || value === null) return false;
  const candidate = value;
  return candidate.version === 1 && typeof candidate.enabled === "boolean" && (candidate.publisherId === null || typeof candidate.publisherId === "string") && typeof candidate.scriptSnippet === "string" && isTextFileManifest(candidate.adsTxt) && isTextFileManifest(candidate.appAdsTxt);
}
function isTextFileManifest(value) {
  if (typeof value !== "object" || value === null) return false;
  const candidate = value;
...
function isIndexNowKeyManifest(value) {
  if (typeof value !== "object" || value === null) return false;
  const candidate = value;
  return typeof candidate.key === "string" && /^[a-f0-9]{32}$/.test(candidate.key);
}
function loadIndexNowKey(baseDir) {
  if (baseDir.includes("\0")) return null;
  if (baseDir.split(/[\\/]+/).includes("..")) return null;
  const normalizedBaseDir = normalize(baseDir);
  const resolvedBaseDir = resolve(normalizedBaseDir);
  ...
  try {
    const parsed = JSON.parse(readFileSync(resolvedManifestPath, "utf8"));
...
const SYSTEM_HOST_SUFFIXES = [".airoapp.ai", ".test-airoapp.ai", ".dev-airoapp.ai", ".dev-godaddy.com"];
function isSystemHost(req) {
  const h = (req.hostname || "").toLowerCase();
  if (!h || h === "localhost" || h === "127.0.0.1") return true;
  return SYSTEM_HOST_SUFFIXES.some((s) => h === s.replace(/^\./, "") || h.endsWith(s));
}
const siteMeta = {
  name: "",
  summary: ""
...
function humanizeLabel(path) {
  ...
  return segment.split(/[-_]/).filter(Boolean).map((word) => word.charAt(0).toUpperCase() + word.slice(1)).join(" ");
}
function sanitizeMarkdown(text) {
  return text.replace(/[\r\n\t]+/g, " ").replace(/[\\`[\]]/g, (c) => `\\${c}`).replace(/ {2,}/g, " ").trim();
}
function renderLlmsTxt(opts) {
  const name = sanitizeMarkdown(opts.name);
  const summary = sanitizeMarkdown(opts.summary);
  const lines = [`# ${name || opts.hostname}`];
  if (summary) {
    lines.push("", `> ${summary}`);
  }
  const pageLinks = opts.routes.filter((r) => typeof r.path === "string" && r.path.startsWith("/")).map((r) => `- [${sanitizeMarkdown(humanizeLabel(r.path))}](${opts.baseUrl}${r.path})`);
...
function llmsTxtHandler(req, res) {
  if (isSystemHost(req)) {
    res.status(404).type("text/plain").set("Cache-Control", "public, max-age=60, must-revalidate").set("Vary", "Host").send("Not found\n");
    return;
  }
...
function registerAdSenseTextRoutes(app2, config) {
...
function renderSsrDocument(template, result, adSenseConfig) {
  const head = [result.head, adSenseConfig.scriptHtml].filter(Boolean).join("\n");
  return template.replace("<!--app-head-->", () => head).replace("<!--app-html-->", () => result.html);
}
...
app.post("/api/contact/:formName", handler$1);
...
`;
  ...
  const base = baseUrl(req);
  const urls = seoRoutes.filter((r) => typeof r.path === "string" && r.path.startsWith("/")).map((r) => {
    ...
    if (r.priority !== void 0) parts.push(`    <priority>${r.priority.toFixed(1)}</priority>`);
    return `  <url>
${parts.join("\n")}
  </url>`;
  }).join("\n");
...
app.get("/llms.txt", llmsTxtHandler);
{
  ...
  app.use(express.static(clientDir, {
    ...
    setHeaders(res, filePath) {
      res.set("Cache-Control", filePath.includes("/assets/") ? "public, max-age=31536000, immutable" : "no-cache");
    }
  }));
  ...
  const fallbackShell = template.replace("<!--app-head-->", "").replace("<!--app-html-->", "");
  let renderFn = null;
  const SSR_MODULE_LOAD_TIMEOUT_MS = 3e4;
  ...
  app.get(/.*/, async (req, res, next) => {
    ...
    const sendFallback = () => res.status(503).set("Content-Type", "text/html; charset=utf-8").set("Cache-Control", "no-store").send(fallbackShell);
    if (renderFn === null) {
      return sendFallback();
    }
    try {
      ...
      if (!result.html) {
        ...
        res.status(result.status).set("Content-Type", "text/html; charset=utf-8").set("Cache-Control", "no-store").send(fallbackShell);
        return;
      }
      const seoHead = isSystemHost(req) ? `<meta name="robots" content="noindex,nofollow">` : `<link rel="canonical" href="${escapeXml(`${req.protocol}://${req.hostname}${req.path}`)}">`;
      const out = renderSsrDocument(template, {
        ...result,
        head: seoHead + result.head
      }, adSenseRuntimeConfig);
      res.status(result.status).set("Content-Type", "text/html; charset=utf-8").set("Cache-Control", "no-cache").send(out);
    } catch (err) {
  ...
  if (!Number.isInteger(port) || port <= 0 || port > 65535) {
    console.error("ssr.server.invalid-port", {
      rawPort
    });
    process.exit(1);
  }
