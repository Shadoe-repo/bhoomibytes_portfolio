...
function directImportError(rel: string): Error {
  return new Error(
    `[airo-content] Direct import of ${toPosix(rel)} is forbidden. ` +
  ...
  return {
    ...
    configureServer(s) {
      const invalidate = (file: string, event: string) => {
        const normFile = normalizePathForComparison(file);
        const normContentDir = normalizePathForComparison(contentDir);
        if (!normFile.startsWith(normContentDir)) return;
    ...
    resolveId(source, importer) {
      ...
      const normResolved = normalizePathForComparison(resolved);
      const normContentDir = normalizePathForComparison(contentDir);

      if (
        (normResolved === normContentDir || normResolved.startsWith(normContentDir + '/')) &&
    ...
    async load(id) {
      ...
      const normCleanId = normalizePathForComparison(cleanId);
      const normContentDir = normalizePathForComparison(contentDir);

      if (
        (normCleanId === normContentDir || normCleanId.startsWith(normContentDir + '/')) &&
...
export async function validateContentEager(
  ...
  for (const entry of entries) {
    ...
    if (!schema) continue;
    if (typeof schema.parse !== 'function') continue;
...
function resolveAbsolute(
  ...
  const normClean = toPosix(clean);

  if (path.isAbsolute(clean) || normClean.startsWith('/')) {
    const rawAbs = path.resolve(clean);
    const normRawAbs = toPosix(rawAbs);
    const rootRel = path.resolve(projectRoot, normClean.startsWith('/') ? normClean.slice(1) : normClean);
    const normRootRel = toPosix(rootRel);
    const normRoot = toPosix(projectRoot);

    return normRawAbs.startsWith(normRoot + '/') ? normRawAbs : normRootRel;
  }

  if (normClean.startsWith('./') || normClean.startsWith('../')) {
    if (!importer) return undefined;
    return toPosix(path.resolve(path.dirname(importer), clean));
  }
  return undefined;
}

function isContentMetaModule(resolvedPath: string, contentDir: string): boolean {
  const normPath = toPosix(resolvedPath);
  const normDir = toPosix(contentDir);
  const rel = toPosix(path.relative(normDir, normPath));
  return (
    rel === 'schemas' ||
    rel === 'schemas.ts' ||
    rel.endsWith('.d.ts')
